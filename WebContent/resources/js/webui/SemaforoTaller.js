...
$(document).ready(function() {
 objTablRO =  $('#TablRO').DataTable( {
         ...
         "columns": [
     	    ...
     	    { "orderable": false },  /*(10)*/
     	    { "orderable": false },   /*(11)*/
     	    { "orderable": false, "visible":false }   /*(11)*/
     	    ],
     	   "createdRow": function ( row, data, index ) {
     		   ...
     		   /* $('td', row).eq(5).addClass('xxbsbg-danger'); */
 ...
 /* table.rows().every( function ( rowIdx, tableLoop, rowLoop ) {
     ...
     console.log("rowLoop:"||rowLoop);
      if ( $(this).hasClass('xxbsbg-danger') ) {
          $(this).removeClass('xxbsbg-danger');
      }
      else {
    	  table.$('tr.xxbsbg-danger').removeClass('xxbsbg-danger');
          $(this).addClass('xxbsbg-danger');
      }
      console.log("Sale"); 
  } );*/
  ...
  document.getElementById("CapturaSemaforoTallerDiv").style.display="none"; 
...
function accionSemaforoTaller(pButton){
	var objformSemaforoTallerId = document.getElementById("formSemaforoTallerId"); 
	var objaccionSemaforoTaller = document.getElementById("accionSemaforoTaller"); 
	
	if("Salir"==pButton){
		objaccionSemaforoTaller.value = "Salir"; 
		objformSemaforoTallerId.submit();
		return; 
	}
	if("Captura"==pButton){
		document.getElementById("SemaforoTallerDiv").style.display="none"; 
	    document.getElementById("CapturaSemaforoTallerDiv").style.display=""; 
...
function accionCaptura(pButton){
	if("Salir"==pButton){
		...
		document.getElementById("SemaforoTallerDiv").style.display=""; 
	    document.getElementById("CapturaSemaforoTallerDiv").style.display="none"; 
	}
	if("Guardar"==pButton){
		document.getElementById("accionFormId").value="CapturaDesdeSemaforoTaller";
	    document.getElementById("submitCaptura").click();
	}
}

function validaCaptura(pForma){
	pForma.descripcionCaptura.value = pForma.descripcionCaptura.value.toUpperCase();
	return true; 
}
...
function imprimeFileExcel(){
	...
	outputText=outputText+'</Style>';
	outputText=outputText+'</Styles>';
	outputText=outputText+'<Worksheet ss:Name="Semaforo Taller">';
	...
	downloadLink.download = "SemaforoTaller.xls";
