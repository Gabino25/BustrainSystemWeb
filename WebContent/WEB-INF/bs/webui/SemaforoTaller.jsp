...
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/webui/SemaforoTaller.js"></script>
</head>
<body>

<jsp:include page="/WEB-INF/commons/header.jsp"/>
<div id="moduloPantallaDiv" class="modPantDiv">
 Archivo - Semaforo Taller
</div> 

<div id="SemaforoTallerDiv">
<table id="TablRO" class="table table-striped table-bordered table-condensed" >
 ...
 <tbody>
  <c:forEach  var="semaforoTaller" items="${listSemaforoTaller}" varStatus="row">
 <tr>
 <td>${semaforoTaller.eco}</td>
 <td>${semaforoTaller.servicioddMMyyyyV2}</td>
 <td>${semaforoTaller.kilometrajemanntoDecimalFormatV3}</td>
 <td>${semaforoTaller.fechakmddMMyyyyV2}</td>
 <td>${semaforoTaller.kilometrajeDecimalFormatV3}</td>
 <td>${semaforoTaller.km_recorridosDecimalFormatV3}</td>
 <td>${semaforoTaller.frecuenciamannto}</td>
 <td>${semaforoTaller.dias_recorridos}</td>
 <td>${semaforoTaller.km_por_dia}</td>
 <td>${semaforoTaller.frecuencia_dias}</td>
 <td>${semaforoTaller.manntosiguienteddMMyyyyV2}</td>
 <td>${semaforoTaller.categoriafiltro}</td>
 </tr>
 </c:forEach>
 </tbody>
 </table>
 
 <form id="formSemaforoTallerId" action="${pageContext.request.contextPath}/SemaforoTallerCO" method="post">
 <input type="hidden" id="accionSemaforoTaller" name="accionSemaforoTaller">
 </form>
 ...
 <td><button id="salirBtn" class="btn btn-primary" onclick="accionSemaforoTaller('Salir');">Salir</button></td>
 <td><button id="capturaBtn"  class="btn btn-primary" onclick="accionSemaforoTaller('Captura');">Captura</button></td>
 <td><button id="excelBtn" class="btn btn-primary" onclick="accionSemaforoTaller('Excel');">Excel</button></td>
 </tr>
 </tbody>
 </table>
 </div>
 
 <div id="CapturaSemaforoTallerDiv">
